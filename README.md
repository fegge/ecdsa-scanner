...
Deploy scanner and PostgreSQL on a single Ubuntu server, secured with Tailscale:
...
This:
- Installs PostgreSQL and builds the scanner
- Configures UFW to only allow Tailscale traffic
- Sets up systemd service
- Restricts SSH to Tailscale only
...
### Two-Node Deployment (Terraform)

For production with separate scanner and database nodes:
...
See [deploy/README.md](deploy/README.md) for all deployment options and configuration.
